...
def main():
    results_dir = Path("results/custom_cnn_sgd")
    ...
    # SGD (no momentum) for Task 10
    # keep simple and comparable
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Custom CNN training (SGD)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(figs_dir / "custom_loss_curve_sgd.png", dpi=200)
    plt.close()
    ...
    print("Training complete (SGD).")
    print("Best model stored at:", best_model_path)
    print("Loss curve stored at: results/figs/custom_loss_curve_sgd.png")
